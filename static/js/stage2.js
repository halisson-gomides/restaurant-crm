...
(function($) {
    ...
    function initializeStage2() {
        ...
        // Auto-initialize step 2 forms if they exist on the page (for direct template loading)
        initializeStep2FormsIfPresent();
    ...
    // Auto-initialize step 2 forms for directly loaded templates
    function initializeStep2FormsIfPresent() {
        const form = $('#registration-form form');
        const recaptchaContainer = $('#recaptcha-container');

        if (form.length > 0 && recaptchaContainer.length > 0) {
            // Get session_id from hidden input
            const sessionId = $('input[name="session_id"]').val();
            const registrationType = window.location.pathname.includes('/cnpj/') ? 'cnpj' : 'cpf';

            if (sessionId && registrationType) {
                ...
                form.on('submit', function(e) {
                    ...
                    const step2Url = `/registration/${registrationType}/step2?session_id=${sessionId}`;
    ...
    function initializeRecaptcha() {
        if (typeof grecaptcha !== 'undefined' && $('#recaptcha-container')) {
            window.recaptchaWidgetId = grecaptcha.render('recaptcha-container', {
                'sitekey': window.recaptchaSiteKey,
                'callback': function(token) {
                    // Success callback
    ...
    window.validateRecaptcha = function() {
        ...
        const errorDiv = $('#recaptcha-error');
    ...
    window.initializeStep2Form = initializeStep2Form;
